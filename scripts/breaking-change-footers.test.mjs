...
//
// Until this file, terraform-suite-ui was the one repository in the estate that
// carried this guard with NO self-test at all -- the exact "green required
// context asserting nothing" shape the other eight had already closed. It was
// found by enumerating the guard from the workflows rather than from the list of
// repositories believed to carry it.
//
...
// deciding anything. It sits beside scripts/strip-sourcemap-sources.test.mjs
// here for the same reason that one exists: a gate nobody exercises is a gate
...
function runGuard(commits, stubDir = binDir) {
  ...
  const result = spawnSync('bash', [scriptPath], {
    ...
    env: {
      ...
      GH_TOKEN: 'stub',
      PR_NUMBER: '123',
      REPO: 'sethbacon/terraform-suite-ui',
...
const FOOTER = 'BREAKING CHANGE: the SuiteThemeProvider props changed'
...
describe('pull requests it must not obstruct', () => {
  it('passes a PR with no breaking change at all', () => {
    const { status, output } = runGuard(['fix: keep the consent banner in sync with stored consent'])
  ...
  it('passes a PR declaring exactly one, which is what the squash can carry', () => {
    const { status, output } = runGuard([`feat: rework the theme provider\n\n${FOOTER}`])
...
describe('pull requests whose squash would drop a declaration', () => {
  it('rejects two footers and says which commits they are in', () => {
    ...
    // changes and documented one.
    const { status, output, summary } = runGuard([
      `feat: drop the legacy theme export\n\n${FOOTER}`,
      'feat: require an explicit locale\n\nBREAKING CHANGE: the implicit locale default is gone',
    ])
  ...
  it('counts the `!` marker the same way as a footer', () => {
    const { status, output, summary } = runGuard([
      'feat!: drop the legacy theme export',
      'fix(i18n)!: require an explicit locale',
    ])
    expect(status).not.toBe(0)
    expect(output).toContain('declares 2 breaking changes')
    expect(summary).toContain('drop the legacy theme export')
    expect(summary).toContain('require an explicit locale')
  })
